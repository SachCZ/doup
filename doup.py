...
def run(tag):
    ...
    os.system('docker exec {} /usr/bin/env bash -c "sudo groupdel docker || true; sudo groupadd -g $(ls -ld /var/run/docker.sock | cut -f4 -d" ") docker; sudo usermod -aG docker devuser;"'.format(tag))
